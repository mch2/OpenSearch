...
public class TransportNativeArrowStreamDataAction extends TransportAction<NativeArrowStreamDataRequest, NativeArrowStreamDataResponse> {
    ...
    private void handleStreamRequest(NativeArrowStreamDataRequest request, TransportChannel channel, Task task) throws IOException {
        // Step 1: Get the channel's allocator. Producer allocators must be children of this
        // so that zero-copy transfer works (Arrow requires shared root allocator).
        ...
        try {
            for (int batch = 0; batch < request.getBatchCount(); batch++) {
                // Step 2: Create a new VectorSchemaRoot per batch from the channel allocator.
                // No child allocator needed — all roots share the same allocator tree,
                // which is required for zero-copy transfer to work.
                VectorSchemaRoot producerRoot = VectorSchemaRoot.create(schema, channelAllocator);

                // Step 3: Populate with typed Arrow vectors — no byte serialization.
                populateBatch(producerRoot, request.getRowsPerBatch(), batch);

                // Step 4: Queue the batch. The framework will zero-copy transfer the
                // producer's buffers into the channel's shared root on the executor thread.
                // After transfer, producerRoot's buffers are moved — don't reuse it.
