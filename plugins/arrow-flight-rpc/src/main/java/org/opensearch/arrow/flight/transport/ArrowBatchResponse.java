 ...
 *
 * <p>The producer creates vectors using a child allocator and populates them freely
 ...
 * <p>Usage (send side):
 * <pre>{@code
 * BufferAllocator child = channelAllocator.newChildAllocator("batch", 0, Long.MAX_VALUE);
 * VectorSchemaRoot producerRoot = VectorSchemaRoot.create(schema, child);
 * // populate producerRoot on any thread...
 * channel.sendResponseBatch(new MyResponse(producerRoot));
 * // producerRoot is consumed — don't reuse it
 * }</pre>
 *
