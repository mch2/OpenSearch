...
public class ArrowBasePlugin extends Plugin implements ExtensiblePlugin, ActionPlugin {
    ...
    ArrowNativeAllocator buildAllocator(Settings settings, ClusterSettings cs, Supplier<Long> budgetSupplier) {
        ...
        // POOL_QUERY is a SPECIAL, UNMANAGED, unbounded pool: arrow-java's C Data importer retains the
        // batch reference BEFORE consulting the allocator, so any throw from allocateBytes permanently
        // leaks it — the limit guarded nothing (zero-copy; memory pre-exists) and real enforcement lives
        // Rust-side (DataFusion pool → CircuitBreakingException → 429). Do NOT add a finite limit or a
        // throwing AllocationListener to this pool.
