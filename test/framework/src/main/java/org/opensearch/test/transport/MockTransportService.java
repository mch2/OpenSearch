...
public final class MockTransportService extends TransportService {
    ...
    public <R extends TransportRequest> void addRequestHandlingBehavior(
        ...
        StubbableTransport streamStub = streamTransportStub();
        if (streamStub != null && streamStub.hasHandler(actionName)) {
            streamStub.addRequestHandlingBehavior(actionName, handlingBehavior);
            return;
        }
    ...
    /**
     * Returns the {@link StubbableTransport} wrapping the streaming transport,
     * or {@code null} if no streaming transport is active.
     */
    private StubbableTransport streamTransportStub() {
        Transport st = getStreamTransport();
        return st instanceof StubbableTransport ? (StubbableTransport) st : null;
    }
