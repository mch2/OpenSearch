...
if [ -n "${CLUSTER_CONFIG:-}" ]; then
  echo "$CLUSTER_CONFIG" >> "$OS_HOME/config/opensearch.yml"
  echo "node.name: ${NODE_NAME:-crash-node}" >> "$OS_HOME/config/opensearch.yml"
  echo "arrow.flight.bind_host: 0.0.0.0" >> "$OS_HOME/config/opensearch.yml"
  echo "arrow.flight.publish_host: _site_" >> "$OS_HOME/config/opensearch.yml"
else
