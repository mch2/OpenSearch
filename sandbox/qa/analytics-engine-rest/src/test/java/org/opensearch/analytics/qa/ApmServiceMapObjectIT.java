...
public class ApmServiceMapObjectIT extends AnalyticsRestTestCase {
    ...
    public void testOperatorMatrixOnParentObject() throws IOException {
        ...
        // to be a no-op (the query-time struct constructor built no validity buffer, so the value was
        // never null), which a non-empty check cannot see — it passed while matching every row. Every fixture row
