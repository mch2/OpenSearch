...
public class CoordinatorResilienceIT extends OpenSearchIntegTestCase {
    ...
    /**
     * Arrow-allocator leak oracle. {@link org.opensearch.analytics.exec.QueryContext#closeBufferAllocator}
     * latches {@link org.opensearch.analytics.exec.QueryContext#LEAK_SIGNAL} whenever its
     * per-query allocator close throws "Memory was leaked". That close runs inside the
     * query-failure listener chain; the resulting {@code IllegalStateException} is
     * attached as a Suppressed on the surfaced error and is often swallowed before it
     * reaches the test. The latch lets us assert out-of-band — regardless of how the
     * in-band exception was routed.
     *
     * <p>Cleared at {@link #clearLeakSignal} as a {@code @Before} hook. Checked in
     * {@link #assertNoArrowLeak} as {@code @After} with a short settling window because
     * the close runs on the failure-listener thread, which is typically another JVM
     * hop away from the test thread returning.
     */
    @Before
    public void clearLeakSignal() {
        org.opensearch.analytics.exec.QueryContext.clearLeakSignal();
    }

    @After
    public void assertNoArrowLeak() throws Exception {
        // Close race: the query's failure listener may still be running when the test
        // body returns (especially on failure paths where the leak probe sits). Poll
        // briefly so we see the signal even if it lands after the test thread exits.
        assertBusy(() -> {
            String leak = org.opensearch.analytics.exec.QueryContext.detectedLeak();
            if (leak != null) {
                fail(
                    "Arrow allocator leak detected during test: "
                        + leak
                        + ". Run with -Darrow.memory.debug.allocator=true (default in this"
                        + " module's ITs) and look in the cluster log for the"
                        + " 'BaseAllocator' + 'QueryContext' error pair — the leak stack"
                        + " follows in the 'live allocations' block."
                );
            }
        }, 5, TimeUnit.SECONDS);
    }
