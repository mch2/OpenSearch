...
pub unsafe extern "C" fn df_create_session_context(
    ...
    let query_config =
        crate::datafusion_query_config::DatafusionQueryConfig::from_ffm_ptr(query_config_ptr);
    // The shard fragment's Substrait plan, used to register the table with the union base_schema
    // up front (null-filling columns this shard omits for index-pattern / alias scans). May be
    // empty when no plan is supplied; widening is then a no-op.
