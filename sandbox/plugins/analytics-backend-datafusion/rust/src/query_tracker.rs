...
/// Yields per flush worker. The abort cascade needs 2-3 scheduling rounds
/// for the full chain: CrossRtStream → PerPartitionStream → SpawnedTask →
/// pull_from_input. 3 yields per worker gives headroom for contention.
