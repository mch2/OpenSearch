...
/**
 * Rewrites {@code TO_DAYS(ts)} → {@code to_unixtime(ts) / 86400 + 719528}.
 * MySQL's day-count is measured from year 0; Unix epoch (1970-01-01) = day 719528.
 */
class ToDaysAdapter implements ScalarFunctionAdapter {
    ...
    public RexNode adapt(RexCall original, List<FieldStorageInfo> fieldStorage, RelOptCluster cluster) {
        if (original.getOperands().size() != 1) {
            throw new IllegalArgumentException("TO_DAYS expects 1 operand, got " + original.getOperands().size());
        }
        ...
        RexNode days = rb.makeCall(bigint, SqlStdOperatorTable.DIVIDE, List.of(epoch, secondsPerDay));
        RexNode offset = rb.makeLiteral(BigDecimal.valueOf(EPOCH_DAY_OFFSET), bigint, false);
        return rb.makeCall(original.getType(), SqlStdOperatorTable.PLUS, List.of(days, offset));
    }
}
