...
public final class NativeBridge {
     ...
     */
    /**
     * Synchronous next-batch pull. The downcall blocks the calling thread inside the native
     * {@code block_on} until the batch (or end-of-stream / error) is ready, then fires the listener
     * inline on the caller. Used by the shard-scan and fetch data-node paths, which run on the
     * SEARCH platform-thread pool. The coordinator-reduce drain uses {@link #streamNextAsync}.
     */
    ...
    public static void streamNextAsync(long runtimePtr, long streamPtr, ActionListener<Long> listener) {
        ...
        try {
            ...
            if (l != null) {
                l.onFailure(convertNativeError(t));
            }
        }
    }
