...
public class ExecutionGraph {
    ...
    private final Map<Integer, AbstractStageExecution> executions;
    private final AbstractStageExecution rootExecution;
    private final List<AbstractStageExecution> leaves;
    private final String queryId;

    ExecutionGraph(
        String queryId,
        Map<Integer, AbstractStageExecution> executions,
        AbstractStageExecution rootExecution,
        List<AbstractStageExecution> leaves
    ) {
     ...
     */
    public static ExecutionGraph build(QueryContext context, StageExecutionBuilder builder, Consumer<AbstractStageExecution> scheduler) {
        Map<Integer, AbstractStageExecution> executions = new HashMap<>();
        Stage rootStage = context.dag().rootStage();
        boolean success = false;
        try {
            AbstractStageExecution rootExec = builder.buildRootExecution(rootStage, context);
            executions.put(rootStage.getStageId(), rootExec);
            List<AbstractStageExecution> leaves = new ArrayList<>();
    ...
    private static void cancelPartialBuild(Map<Integer, AbstractStageExecution> executions) {
        IllegalStateException primary = null;
        for (AbstractStageExecution stage : executions.values()) {
            try {
    ...
    private static void buildChildrenRecursively(
        Consumer<AbstractStageExecution> scheduler,
        Map<Integer, AbstractStageExecution> executions,
        StageExecutionBuilder builder,
        AbstractStageExecution parentExec,
        Stage parentStage,
        QueryContext config
    ) {
        ...
        List<AbstractStageExecution> childExecs = new ArrayList<>(children.size());
        for (Stage child : children) {
            AbstractStageExecution childExec = builder.buildExecution(child, parentExec, config);
    ...
    private static void collectLeaves(
        ...
        if (stage.getChildStages().isEmpty()) {
            AbstractStageExecution exec = executions.get(stage.getStageId());
            if (exec != null) {
                leaves.add(exec);
            }
        } else {
    ...
    public AbstractStageExecution rootExecution() {
        return rootExecution;
    }

    /** All leaf executions (stages with no children). */
    public List<AbstractStageExecution> leaves() {
        return Collections.unmodifiableList(leaves);
    }

    /** All stage executions in the graph. */
    public Collection<AbstractStageExecution> allExecutions() {
    ...
    public String explain() {
        ...
        for (AbstractStageExecution exec : executions.values()) {
