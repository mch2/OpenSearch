...
public final class ResultHeapCharge implements Releasable {
    ...
    /** Charge label {@code <analytics_result[queryId]>} for log/trip attribution. */
    String label() {
        return label;
    }

    /** Native→heap expansion factor, so callers can compare an actual native size to {@link #chargedBytes()}. */
    double expansionFactor() {
        return heapExpansionFactor;
    }
