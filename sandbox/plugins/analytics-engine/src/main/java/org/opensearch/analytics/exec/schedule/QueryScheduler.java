...
public class QueryScheduler implements Scheduler {
    ...
    /**
     * Materialises {@code stage}'s tasks via {@link AbstractStageExecution#start()}, then dispatches
     * each through the stage's {@link StageTaskRunner} with a scheduler-owned {@link ActionListener}.
     ...
     */
    void scheduleStage(@MonotonicNonNull AbstractStageExecution stage) {
        stage.start();
        ...
        StageTaskRunner<StageTask> runner = (StageTaskRunner<StageTask>) stage.taskRunner();
    ...
    /**
     * On failure, retries via {@link AbstractStageExecution#retargetForRetry} if the stage hands
     ...
     */
    protected ActionListener<Void> handleFor(AbstractStageExecution stage, StageTask task) {
     ...
     * custom {@link org.opensearch.analytics.exec.schedule.StageExecutionFactory} for a stage
