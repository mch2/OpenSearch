...
public abstract class AbstractStageExecution implements StageExecution {
    ...
    protected StageTaskRunner<?> runner = StageTaskRunner.NONE;
     ...
     * scheduler. Package-private: only {@link QueryScheduler} drives stage start.
     */
    final void start() {
        List<StageTask> ts;
        try {
    ...
    /**
     * Default per-task terminal handler: fast-fail on any failure, otherwise decrement the
     * pending counter and transition to SUCCEEDED/FAILED once every task is terminal.
     * Retry intercepts BEFORE this fires — see {@link QueryScheduler#handleFor}.
     * Protected so subclasses may override; called only from {@link QueryScheduler}.
     */
    protected void onTaskTerminal(StageTask task, @Nullable Exception cause) {
        if (cause != null) {
     ...
     * attempt budget, retryable exception classes, alternate selection.
     * {@link QueryScheduler#handleFor} consults this on every failure.
     */
    protected Optional<StageTask> retargetForRetry(StageTask failed, Exception cause) {
        return Optional.empty();
    }

    /**
     * Control-plane payload (broadcast bytes, per-shard stats) handed to the parent via
     * {@link #consumeChildMetadata} before the parent is scheduled. Default null.
     */
    @Nullable
    ...
    /**
     * Per-input EOF hook fired by the cascade on every child SUCCEEDED. Default no-op;
     * streaming reduce overrides to close the just-finished child's sender.
     */
    ...
    /**
     * Capture cause, transition to FAILED. Stage-internal cleanup runs via
     * {@link #onTerminalTransition}. Package-private — only the cascade invokes it.
     */
    boolean failWithCause(Exception cause) {
        captureFailure(cause);
     ...
     * Package-private — only {@link ExecutionGraph} calls this.
     */
    final void attachChildren(List<? extends AbstractStageExecution> children, Consumer<AbstractStageExecution> scheduler) {
        if (children.isEmpty()) return;

        boolean eager = schedulesEagerly();
        AtomicInteger pending = new AtomicInteger(children.size());
        AtomicInteger eagerScheduled = new AtomicInteger(0);
        for (AbstractStageExecution child : children) {
                ...
                switch (to) {
                    ...
                    case SUCCEEDED -> {
                        ...
                        if (pending.decrementAndGet() == 0) {
                            ...
                            for (AbstractStageExecution c : children) {
