...
public class AnalyticsQueryService extends AbstractLifecycleComponent {
    ...
    private final ConcurrentMapLong<ExecutionContext> activeContexts =
     ...
     */
    public Iterable<Object[]> execute(ResolvedPlan plan, IndexShard shard) {
        ...
        if (plugin == null) {
            throw new IllegalStateException(
                "No plugin registered for backend [" + plan.getPrimaryBackend() + "]");
        }

        String tableName = plan.getRoot().getTable() != null
            ? plan.getRoot().getTable().getQualifiedName().get(
                plan.getRoot().getTable().getQualifiedName().size() - 1)
            : "unknown";

        ExecutionContext ctx = new ExecutionContext(plan, tableName);
        long ctxId = putContext(ctx);

        try (DataFormatAwareEngine.DataFormatAwareReader dataFormatAwareReader =
                 dataFormatAwareEngine.acquireReader()) {

            // Set up delegation from plan predicates
            DelegationContext delegationContext = setUpDelegation(plan, ctx, dataFormatAwareReader);
            ctx.setDelegationContext(delegationContext);

            // Create primary engine and execute
            SearchExecEngine engine = plugin.searcher(ctx, dataFormatAwareReader);
            ...
            // Release delegation targets
            delegationContext.release();

            logger.info("[AnalyticsQueryService] Completed via [{}], {} rows, ctxId={}",
                plugin.name(), rows.size(), ctxId);
            return rows;
        } catch (Exception e) {
    ...
    private DelegationContext setUpDelegation(ResolvedPlan resolved, ExecutionContext ctx,
                                              DataFormatAwareEngine.DataFormatAwareReader dataFormatAwareReader) {
        Map<String, RexNode> delegationPredicates = resolved.getDelegationPredicates();
        if (delegationPredicates.isEmpty()) return DelegationContext.NONE;

        DelegationBroker broker = DelegationBroker.getInstance();
        List<Long> ids = new ArrayList<>();

        for (Map.Entry<String, RexNode> entry : delegationPredicates.entrySet()) {
            String targetName = entry.getKey();

            AnalyticsSearchBackendPlugin targetPlugin = backEnds.get(targetName);
            if (targetPlugin == null) {
                throw new IllegalStateException(
                    "No plugin registered for delegation target [" + targetName + "]");
            }

            SearchExecEngine targetEngine = targetPlugin.searcher(ctx, dataFormatAwareReader);
            DelegationTarget target = targetPlugin.getDelegationTarget(DelegationType.FILTER, targetEngine);
            if (target == null) {
                throw new IllegalStateException(
                    "Backend [" + targetName + "] does not support FILTER delegation");
            }

            long id = broker.register(target);
            ids.add(id);
            logger.info("Registered delegation target for backend [{}], id={}", targetName, id);
        }

        return new DelegationContext(ids);
    }
