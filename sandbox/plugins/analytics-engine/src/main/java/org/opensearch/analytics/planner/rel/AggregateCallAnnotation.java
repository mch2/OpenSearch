...
public class AggregateCallAnnotation extends RexCall implements OperatorAnnotation {
    ...
    public AggregateCallAnnotation(RelDataType type, List<String> viableBackends, int annotationId) {
        super(type, AGG_CALL_ANNOTATION_OP, List.of());
        this.viableBackends = viableBackends;
        this.annotationId = annotationId;
    }

    /** Convenience constructor — uses BOOLEAN as the marker type. Used by callers that
     *  store annotations in a side map keyed by AggregateCall index (not in rexList). */
