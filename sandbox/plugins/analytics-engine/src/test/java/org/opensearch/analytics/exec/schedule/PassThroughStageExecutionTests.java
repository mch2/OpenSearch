...
import org.opensearch.analytics.exec.schedule.coordinator.LocalStageTask;
...
public class PassThroughStageExecutionTests extends OpenSearchTestCase {
    ...
    public void testStartMaterialisesSingleLocalTask() {
        // Typed as AbstractStageExecution so the package-private start/tasks hooks reach
        // this test from the schedule package; the concrete type still owns them.
        AbstractStageExecution exec = new PassThroughStageExecution(stageWithId(0), mockContext(), new RowProducingSink());
        exec.start();
