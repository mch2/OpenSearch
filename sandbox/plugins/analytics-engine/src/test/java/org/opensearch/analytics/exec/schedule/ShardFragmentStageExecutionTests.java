...
import org.opensearch.analytics.exec.schedule.shard.ShardFragmentStageExecution;
...
public class ShardFragmentStageExecutionTests extends OpenSearchTestCase {
    ...
    public void testFastFailsOnFirstTaskFailureWithoutWaitingForSiblings() {
        CapturingSink sink = new CapturingSink();
        // Typed as AbstractStageExecution so the package-private hooks (start, tasks,
        // onTaskTerminal) are reachable from this test package.
        AbstractStageExecution exec = buildExecutionWithTargets(sink, 3);
        exec.start();
     ...
     */
    private static void scheduleAndDispatch(AbstractStageExecution exec) {
        exec.start();
        @SuppressWarnings("unchecked")
        StageTaskRunner<StageTask> dispatcher = (StageTaskRunner<StageTask>) exec.taskRunner();
