 ...
 * <ul>
 *   <li>{@link ShardScanStageExecution} implements {@link StageExecution}</li>
...
public class StageExecutionInterfaceTests extends OpenSearchTestCase {
    ...
    /**
     * ShardScanStageExecution must implement the StageExecution interface.
     */
    public void testFanOutImplementsInterface() {
        assertTrue(
            "ShardScanStageExecution must implement StageExecution",
            StageExecution.class.isAssignableFrom(ShardScanStageExecution.class)
        );
    }
    ...
    /**
     * The State enum must contain all expected values.
     */
    public void testAllStatesPresent() {
        StageExecution.State[] states = StageExecution.State.values();
        assertEquals("State enum must have 5 values", 5, states.length);
