...
public class ResultHeapChargeTests extends org.opensearch.test.OpenSearchTestCase {
    ...
    /** No-op and idempotency guards: non-positive charge, double close, and charge-after-close. */
    public void testNoopAndIdempotencyGuards() {
        CircuitBreaker breaker = requestBreaker(1_000_000);

        // Non-positive charge is a no-op.
        ResultHeapCharge nonPos = new ResultHeapCharge(breaker, "q-nonpos", 1.0);
        nonPos.charge(0);
        nonPos.charge(-5);
        assertEquals(0, nonPos.chargedBytes());
        assertEquals(0, breaker.getUsed());

        // close is idempotent — a second close does not double-release.
        ResultHeapCharge idem = new ResultHeapCharge(breaker, "q-idem", 1.0);
        idem.charge(100);
        assertEquals(100, breaker.getUsed());
        idem.close();
        idem.close();
        assertEquals(0, idem.chargedBytes());
        assertEquals(0, breaker.getUsed());

        // Charging after close must not re-open the reservation.
        ResultHeapCharge afterClose = new ResultHeapCharge(breaker, "q-afterclose", 1.0);
        afterClose.close();
        afterClose.charge(100);
        assertEquals(0, afterClose.chargedBytes());
        assertEquals(0, breaker.getUsed());
    }
