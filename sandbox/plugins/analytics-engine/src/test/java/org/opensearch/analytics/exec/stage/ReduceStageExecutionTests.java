...
public class ReduceStageExecutionTests extends OpenSearchTestCase {
    ...
    public void testChildFailureClosesChildInputBeforeFailingParent() {
        ...
        assertTrue("closeChildInput must be called for the failed child",
            backend.closedChildIds.contains(childStageId));
        assertEquals(StageExecution.State.FAILED, exec.getState());
    ...
    private static final class FakeChildExecution implements StageExecution {
        ...
        @Override public int getStageId() { return stage.getStageId(); }
        @Override public State getState() { return state; }
        @Override public Exception getFailure() { return failure; }
        @Override public void start() { state = State.RUNNING; }
        @Override public void addStateListener(StageStateListener listener) { listeners.add(listener); }
        @Override public void cancel(String reason) { state = State.CANCELLED; }
    }
}
