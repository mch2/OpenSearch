...
public class DefaultQueryPlannerResolveTests extends OpenSearchTestCase {
    ...
    public void testResolutionPriorityOrder() {
        for (int i = 0; i < 100; i++) {
            ...
            assertEquals("highest-priority backend must be selected", "first", result.getPrimaryBackend());
        }
    }
    ...
    public void testNoUnresolvedAfterResolution() {
        for (int i = 0; i < 100; i++) {
            ...
            LogicalTableScan scan = buildScan();
            ResolvedPlan result = planner.plan(scan, 1);
            assertNotEquals("root must not be unresolved", "unresolved", result.getPrimaryBackend());
    ...
    public void testUnresolvedRexNodeResolved() {
        ...
        var plugin = mock(org.opensearch.analytics.spi.AnalyticsSearchBackendPlugin.class);
    ...
    public void testHybridFilterCreated() {
        ...
        Map<String, RexNode> predicates = java.util.Map.of("lucene", condition, "datafusion", condition);
