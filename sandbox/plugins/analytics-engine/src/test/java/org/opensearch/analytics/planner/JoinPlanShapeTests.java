...
public class JoinPlanShapeTests extends PlanShapeTestBase {
    ...
    public void testCrossJoin_2shard() {
        ...
        RelNode join = LogicalJoin.create(stubScan(left), stubScan(right), List.of(), trueCondition, Set.<CorrelationId>of(), JoinRelType.INNER);
        RelNode result = runPlanner(join, context);
        assertPlanShape(
            """
                OpenSearchJoin(condition=[true], joinType=[inner], viableBackends=[[mock-parquet]], strategy=[CoordinatorHashJoin])
                  OpenSearchExchangeReducer(viableBackends=[[mock-parquet]], exchange=[ExchangeInfo[distributionType=SINGLETON, partitionKeyIndices=[]]])
                    OpenSearchTableScan(table=[[left_idx]], viableBackends=[[mock-parquet]])
                  OpenSearchExchangeReducer(viableBackends=[[mock-parquet]], exchange=[ExchangeInfo[distributionType=SINGLETON, partitionKeyIndices=[]]])
                    OpenSearchTableScan(table=[[right_idx]], viableBackends=[[mock-parquet]])
    ...
    private void runJoinKindShape(JoinRelType joinType, String label) {
        ...
        assertPlanShape(
            """
                OpenSearchJoin(condition=[=($0, $2)], joinType=[__JOIN_TYPE__], viableBackends=[[mock-parquet]], strategy=[CoordinatorHashJoin])
                  OpenSearchExchangeReducer(viableBackends=[[mock-parquet]], exchange=[ExchangeInfo[distributionType=SINGLETON, partitionKeyIndices=[]]])
                    OpenSearchTableScan(table=[[left_idx]], viableBackends=[[mock-parquet]])
                  OpenSearchExchangeReducer(viableBackends=[[mock-parquet]], exchange=[ExchangeInfo[distributionType=SINGLETON, partitionKeyIndices=[]]])
                    OpenSearchTableScan(table=[[right_idx]], viableBackends=[[mock-parquet]])
