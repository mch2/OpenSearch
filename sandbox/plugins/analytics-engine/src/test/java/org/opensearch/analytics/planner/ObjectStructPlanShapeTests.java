 ...
 * above the scan with {@code get_field}.
 *
...
public class ObjectStructPlanShapeTests extends PlanShapeTestBase {
     ...
     * struct, nesting a second {@code get_field} for the {@code properties} sub-object.
     */
    public void testProjectorReadsLeavesOutOfTheStructAboveTheScan() {
        ...
        assertPlanShape(
            """
                LogicalProject(id=[$0], nested_metadata.top=[get_field($1, 'top':VARCHAR)], nested_metadata.properties.name=[get_field(get_field($1, 'properties':VARCHAR), 'name':VARCHAR)], nested_metadata.properties.value=[get_field(get_field($1, 'properties':VARCHAR), 'value':VARCHAR)], nested_metadata=[$1])
    ...
    public void testProjectOnOneLeafReadsOnlyThatLeaf() {
        ...
        assertTrue("expected the leaf to be read out of the struct, got:\n" + shape, shape.contains("get_field"));
        assertEquals("only the requested leaf should be read, got:\n" + shape, 1, shape.split("get_field", -1).length - 1);
    }
