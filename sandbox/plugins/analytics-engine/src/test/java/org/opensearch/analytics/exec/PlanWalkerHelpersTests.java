...
/**
 * Tests for {@link Stage} computed properties: tableName, isShuffleWrite.
...
public class PlanWalkerHelpersTests extends OpenSearchTestCase {
    ...
    public void testLocalInferredForNoExchangeNoTableScan() {
        OpenSearchStageInputScan stageInput = new OpenSearchStageInputScan(cluster, RelTraitSet.createEmpty(), 0, rowType, List.of());
        Stage stage = new Stage(1, stageInput, List.of(), null);
        assertEquals(StageExecutionType.LOCAL, stage.getExecutionType());
    }

    public void testDataNodeInferredWithTableScan() {
        OpenSearchTableScan scan = buildTableScan("http_logs", List.of("lucene"));
        Stage stage = new Stage(0, scan, List.of(), null);
        assertEquals(StageExecutionType.DATA_NODE, stage.getExecutionType());
    }

    public void testDataNodeInferredWithExchange() {
        OpenSearchTableScan scan = buildTableScan("http_logs", List.of("lucene"));
