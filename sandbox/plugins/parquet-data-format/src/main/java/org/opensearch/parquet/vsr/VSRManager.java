...
public class VSRManager implements AutoCloseable {
    ...
    private boolean reconcileField(ManagedVSR activeVSR, Field schemaField, String parentPath) {
        ...
        if (isObject == false) {
            FieldVector existingVector = activeVSR.getVector(path);
            if (existingVector != null) {
                if (hasSameStorageShape(existingVector.getField(), schemaField) == false) {
                    throw new IllegalStateException(
                        "Field ["
                            + path
            ...
            // Pass the schema field through as-is: rebuilding it from name + FieldType alone would
            // drop getChildren(), leaving a LIST column with no element vector.
            if (parentPath == null) {
