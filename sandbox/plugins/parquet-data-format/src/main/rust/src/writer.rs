...
impl NativeParquetWriter {
    ...
    pub(crate) fn sort_batch(
        ...
        // Resolve each key to its array first, which descends an `object` to the named leaf, then
        // reduce it: a LIST sorts by its min (or max, descending), which is what Lucene does for a
        // multi-valued field. Taking the SortField type from the reduced array rather than from the
        // schema keeps the declared type and the values from disagreeing.
        let sort_arrays: Vec<Arc<dyn arrow::array::Array>> = sort_columns
            .iter()
            .enumerate()
            .map(|(i, col_name)| {
                let array = resolve_sort_array(batch, col_name)
                    .ok_or_else(|| -> Box<dyn std::error::Error> {
                        format!("Sort column '{}' not found in schema", col_name).into()
                    })?;
                let max = max_sort_modes.get(i).copied().unwrap_or(false);
                reduced_sort_array(&array, max).map_err(|e| -> Box<dyn std::error::Error> { e.into() })
            })
...
mod sort_key_tests {
    ...
    /// `id`, an `object` with a `name` leaf and a `props.zone` sub-leaf, and a column whose own name
    /// contains dots.
    fn batch_with_object() -> RecordBatch {
        let inner = Fields::from(vec![Field::new("zone", DataType::Utf8, true)]);
        let city = Fields::from(vec![
            Field::new("name", DataType::Utf8, true),
            Field::new("props", DataType::Struct(inner.clone()), true),
        ]);
        let props: ArrayRef = Arc::new(
            StructArray::try_new(
                inner,
                vec![Arc::new(StringArray::from(vec!["west"])) as ArrayRef],
                None,
            )
            .unwrap(),
        );
        let city_array: ArrayRef = Arc::new(
            StructArray::try_new(
                city.clone(),
                vec![
        ...
        let schema = Arc::new(Schema::new(vec![
            ...
            Field::new("city", DataType::Struct(city), true),
            // One column, dots and all — a keyword's derived-source companion looks like this.
            Field::new("_ignored_source.city.name", DataType::Utf8, true),
        ]));
        RecordBatch::try_new(
            schema,
            vec![
                ...
                city_array,
                Arc::new(StringArray::from(vec!["raw"])) as ArrayRef,
            ],
        )
        .unwrap()
    }
    ...
    #[test]
    fn a_top_level_sort_column_resolves_directly() {
        let batch = batch_with_object();
        let array = resolve_sort_array(&batch, "id").expect("id is a column");
        assert_eq!(array.data_type(), &DataType::Int32);
    }

    /// `index.parquet.sort.field: [city.name]` — a leaf of an object. It is a child of the `city`
    /// group rather than a column, so resolving it has to descend.
    #[test]
    fn an_object_leaf_resolves_as_a_sort_key() {
        let batch = batch_with_object();
        let array = resolve_sort_array(&batch, "city.name").expect("city.name is a leaf of city");
        let values = array.as_any().downcast_ref::<StringArray>().unwrap();
        assert_eq!(values.value(0), "seattle");
    }

    #[test]
    fn a_sub_object_leaf_resolves_as_a_sort_key() {
        let batch = batch_with_object();
        let array = resolve_sort_array(&batch, "city.props.zone").expect("resolves at depth");
        let values = array.as_any().downcast_ref::<StringArray>().unwrap();
        assert_eq!(values.value(0), "west");
    }

    /// A column whose own name contains dots wins over descending: it is matched whole first, so it
    /// is not mistaken for a path into something.
    #[test]
    fn a_dotted_column_name_is_matched_whole() {
        let batch = batch_with_object();
        let array = resolve_sort_array(&batch, "_ignored_source.city.name").expect("one column");
        let values = array.as_any().downcast_ref::<StringArray>().unwrap();
        assert_eq!(values.value(0), "raw");
    }

    #[test]
    fn an_absent_sort_column_resolves_to_nothing() {
        let batch = batch_with_object();
        assert!(resolve_sort_array(&batch, "city.missing").is_none());
        assert!(resolve_sort_array(&batch, "nope").is_none());
    }
}
