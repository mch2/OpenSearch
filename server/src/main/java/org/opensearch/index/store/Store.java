...
public class Store extends AbstractIndexShardComponent implements Closeable, RefCounted {
    ...
    private void cleanupFiles(String reason, MetadataSnapshot localSnapshot, @Nullable Collection<String> additionalFiles)
        ...
        for (String existingFile : directory.listAll()) {
            final boolean b = additionalFiles != null && additionalFiles.contains(existingFile);
            final boolean contains = localSnapshot.contains(existingFile);
            if (Store.isAutogenerated(existingFile)
                || contains
                || b) {
                // don'u delete snapshot file, or the checksums file (note, this is extra protection since the Store won't delete
                // checksum)
                if (existingFile.startsWith("_0.cfe")) {
                    logger.info("Skipping delete {} {} {}", existingFile, b, contains);
                }
                continue;
            }
            try {
                if (existingFile.startsWith("_0.cfe")) {
                    logger.info("deleting {} {} {}", existingFile, b, contains);
                }
                directory.deleteFile(reason, existingFile);
            } catch (IOException ex) {
                if (existingFile.startsWith(IndexFileNames.SEGMENTS) || existingFile.startsWith(CORRUPTED_MARKER_NAME_PREFIX)) {
