 ...
 */
public class SegmentReplicationSourceService extends AbstractLifecycleComponent implements IndexEventListener, ClusterStateListener {

    private static final Logger logger = LogManager.getLogger(SegmentReplicationSourceService.class);

    public static class Actions {
        public static final String START_RECOVERY = "internal:index/shard/segrep/start_recovery";
        public static final String REESTABLISH_RECOVERY = "internal:index/shard/segrep/reestablish_recovery";
    }
    ...
    @Inject
    public SegmentReplicationSourceService(TransportService transportService, IndicesService indicesService, RecoverySettings recoverySettings) {
    ...
    private void recover(StartRecoveryRequest request, ActionListener<RecoveryResponse> listener) {
        ...
        try {
            final SequenceNumbers.CommitInfo seqNoStats = SequenceNumbers.loadSeqNoInfoFromLuceneCommit(
                shard.store().readLastCommittedSegmentsInfo().userData.entrySet()
            );
            logger.info("Last committed seqno {}", seqNoStats);
        } catch (IOException e) {
            e.printStackTrace();
        }
        ...
        SegmentReplicationSourceHandler handler = ongoingRecoveries.addNewRecovery(request, shard);
        logger.trace(
    ...
    final class OngoingRecoveries {
        ...
        private final Map<DiscoveryNode, Collection<RemoteSegmentReplicationTargetHandler>> nodeToHandlers = new HashMap<>();

        @Nullable
        private List<ActionListener<Void>> emptyListeners;

        synchronized SegmentReplicationSourceHandler addNewRecovery(StartRecoveryRequest request, IndexShard shard) {
            final ShardRecoveryContext shardContext = ongoingRecoveries.computeIfAbsent(shard, s -> new ShardRecoveryContext());
            final Tuple<SegmentReplicationSourceHandler, RemoteSegmentReplicationTargetHandler> handlers = shardContext.addNewRecovery(request, shard);
            final RemoteSegmentReplicationTargetHandler recoveryTargetHandler = handlers.v2();
        ...
        synchronized void cancelOnNodeLeft(DiscoveryNode node) {
            final Collection<RemoteSegmentReplicationTargetHandler> handlers = nodeToHandlers.get(node);
            if (handlers != null) {
                for (RemoteSegmentReplicationTargetHandler handler : handlers) {
                    handler.cancel();
                }
            }
        }
        ...
        synchronized void remove(IndexShard shard, SegmentReplicationSourceHandler handler) {
            ...
            final RemoteSegmentReplicationTargetHandler removed = shardRecoveryContext.recoveryHandlers.remove(handler);
        ...
        synchronized void cancel(IndexShard shard, String reason) {
            ...
            if (shardRecoveryContext != null) {
                ...
                for (SegmentReplicationSourceHandler handlers : shardRecoveryContext.recoveryHandlers.keySet()) {
                    try {
        ...
        private final class ShardRecoveryContext {
            final Map<SegmentReplicationSourceHandler, RemoteSegmentReplicationTargetHandler> recoveryHandlers = new HashMap<>();

            /**
             * Adds recovery source handler.
             */
            synchronized Tuple<SegmentReplicationSourceHandler, RemoteSegmentReplicationTargetHandler> addNewRecovery(
                StartRecoveryRequest request,
                IndexShard shard
            ) {
                for (SegmentReplicationSourceHandler existingHandler : recoveryHandlers.keySet()) {
                ...
                final Tuple<SegmentReplicationSourceHandler, RemoteSegmentReplicationTargetHandler> handlers = createRecoverySourceHandler(request, shard);
            ...
            synchronized void reestablishRecovery(ReestablishRecoveryRequest request, ActionListener<RecoveryResponse> listener) {
                SegmentReplicationSourceHandler handler = null;
                for (SegmentReplicationSourceHandler existingHandler : recoveryHandlers.keySet()) {
            ...
            private Tuple<SegmentReplicationSourceHandler, RemoteSegmentReplicationTargetHandler> createRecoverySourceHandler(
                StartRecoveryRequest request,
                IndexShard shard
            ) {
                SegmentReplicationSourceHandler handler;
                final RemoteSegmentReplicationTargetHandler recoveryTarget = new RemoteSegmentReplicationTargetHandler(
                    request.recoveryId(),
                    request.shardId(),
                    transportService,
                    request.targetNode(),
                ...
                handler = new SegmentReplicationSourceHandler(
                    shard,
                    recoveryTarget,
                    shard.getThreadPool(),
                    request,
