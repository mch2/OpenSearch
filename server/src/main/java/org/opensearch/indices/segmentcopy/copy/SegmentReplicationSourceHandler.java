...
public class SegmentReplicationSourceHandler {
     ...
     */
    public void getFiles(NRTCopyState copyState, ActionListener<GetFilesResponse> listener) {
        ...
        try {
            ...
            // As soon as we finish sending infos bytes, we can release our copyState ref.
            try {
                // TODO: Segrep - if a file is missing here we need to throw/alert caller.
                final StoreFileMetadata[] storeFileMetadata = request.getFilesToFetch().stream()
                    .map(file -> copyState.getMetadataSnapshot().get(file.name())).toArray(StoreFileMetadata[]::new);

                sendFiles((Store) acquireStore(shard.store()), storeFileMetadata, sendFileStep);
